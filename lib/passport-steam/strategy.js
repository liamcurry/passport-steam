...
function Strategy(options, validate) {
  ...
  options.profile =  options.profile || true;
  options.stateless = true; //Steam only works as a stateless OpenID

  if(typeof arguments[0].query !== 'undefined' && arguments[0].query['openid.op_endpoint'] !== 'https://steamcommunity.com/openid/login')
    return done('Claimed identity is invalid');
  ...
  } else OpenIDStrategy.call(this, options, validate);
